...
const Tooltip = ({ 
      ...
      const calculatePosition = () => {
        ...
        switch (placement) {
          case 'top':
            ...
            break;
          case 'bottom':
            if (positions.bottom + tooltipRect.height > viewport.height) newPlacement = 'top';
            break;
          case 'left':
            if (positions.left < 0) newPlacement = 'right';
            break;
          case 'right':
            if (positions.right + tooltipRect.width > viewport.width) newPlacement = 'left';
            break;
        }
        ...
        if ((newPlacement === 'left' || newPlacement === 'right')) {
          ...
          if (centerY - tooltipRect.height / 2 < 0 || centerY + tooltipRect.height / 2 > viewport.height) {
  ...
  const getTooltipClasses = () => {
    const baseClasses = "absolute z-[9999] px-3 py-2 text-xs font-medium transition-all duration-200 transform";
    const themeClasses = "bg-black/90 text-theme-primary border border-theme-primary/30 rounded-lg backdrop-blur-sm";
  ...
  const getArrowClasses = () => {
    const baseClasses = "absolute w-2 h-2 bg-black/90 border border-theme-primary/30 transform rotate-45";
    ...
    switch (actualPlacement) {
      case 'top':
        ...
        break;
      case 'bottom':
        positionClasses = "bottom-full left-1/2 -translate-x-1/2 -mb-1 border-b-0 border-r-0";
        break;
      case 'left':
