...
export const ThemeTitle = ({ children, className, level = 1, ...props }) => {
    ...
    return (
        <Tag className={cn(baseClasses, levelClasses[level], className)} {...props}>
            {children}
        </Tag>
    );
};
...
export const ThemeDescription = ({ children, className, ...props }) => {
    return (
        <p
            className={cn('text-theme-text-muted transition-colors duration-300', className)}
            {...props}
        >
            {children}
        </p>
...
export const ThemeButton = ({
    ...
    const baseClasses =
        'theme-button font-medium rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-theme-primary/50 backdrop-blur-sm';

    const variantClasses = {
        primary:
            'bg-theme-button-primary text-white hover:bg-theme-button-hover border border-theme-primary/30',
        secondary:
            'bg-theme-surface border border-theme-border-neutral text-theme-text-primary hover:bg-theme-surface-elevated', // 使用中性边框
        outline:
            'border-2 border-theme-button-border text-theme-text-primary hover:bg-theme-button-hover bg-theme-button',
...
export const ThemeIcon = ({ children, className, ...props }) => {
    return (
        <span
            className={cn('theme-icon inline-flex items-center justify-center', className)}
            {...props}
        >
            {children}
        </span>
...
export const ThemeDivider = ({ className, vertical = false, ...props }) => {
    ...
    return <div className={cn(baseClasses, orientationClasses, className)} {...props} />;
};
...
ThemeDivider.propTypes = {
    className: PropTypes.string,
    vertical: PropTypes.bool,
};

ThemeNumber.propTypes = {
    children: PropTypes.node.isRequired,
    className: PropTypes.string,
};
