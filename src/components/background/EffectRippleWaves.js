...
export class EffectRippleWaves {
    ...
    createWaveSources() {
        ...
        this.sourceGeometry.setAttribute('position', new THREE.Float32BufferAttribute(sourcePositions, 3));
    ...
    calculateInterference(currentWave, currentMesh) {
        ...
        if (currentMesh.userData.isShaderRipple && currentMesh.material.uniforms) {
            ...
            currentMesh.material.uniforms.opacity.value = Math.min(1.0, baseOpacity + interferenceBoost);
            
            // 干涉时增强颜色亮度
            if (interferenceBoost > 0) {
                const brightColor = this.waveColor.clone().multiplyScalar(1 + interferenceBoost * 0.5);
                currentMesh.material.uniforms.primaryColor.value.lerp(brightColor, 0.3);
            }
        } else {
            // 传统材质：直接修改opacity
            currentMesh.material.opacity = Math.min(0.8, currentMesh.material.opacity + interferenceBoost);
        }
    }
