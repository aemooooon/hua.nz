    ...
    useEffect(() => {
        ...
        const addTestPaintings = (scene, loadingManager = null) => {
            ...
            const analyzeImageDimensions = async () => {
                    ...
                    if (item.src || item.thumbnail) {
                        try {
                            ...
                            if (item.aspectRatio && item.dimensions) {
                                ...
                                console.log(
                                    `✅ 使用预计算尺寸: ${item.id} - ${dimensions.width}x${dimensions.height} (${aspectRatio.toFixed(2)})`
                                );
                            } else {
                                // 🔄 降级到动态尺寸检测
                ...
                console.log(
                    `📊 尺寸分析完成: ${precomputedCount}/${totalCount} 使用预计算数据 (${Math.round((precomputedCount / totalCount) * 100)}%)`
                );
            ...
            const assignPaintingsToWalls = imageAnalysis => {
                ...
                // 排除灯箱使用的图片 - 不能出现在墙面上
                const filteredAnalysis = imageAnalysis.filter(
                    img => img.item.position !== 'lightbox'
                );

                // 按store中预设的wall属性分组
                const verticalWallImages = filteredAnalysis.filter(
                    img => img.item.wall === 'vertical_wall_32m'
                );
                const horizontalWallImages = filteredAnalysis.filter(
                    img => img.item.wall === 'horizontal_wall_64m'
                );

                // 为了实现左右墙对称，需要重新分组横向图片
                // 按layer分组，确保左右墙都有upper和lower画作
                const upperLayerImages = horizontalWallImages.filter(
                    img => img.item.layer === 'upper'
                );
                const lowerLayerImages = horizontalWallImages.filter(
                    img => img.item.layer === 'lower'
                );
            ...
            const createPaintingsAsync = async () => {
                // 🚫 防止重复创建画作
                if (
                    scene.children.some(child => child.name && child.name.startsWith('painting_'))
                ) {
                ...
                const createPaintingAtPosition = async (imageData, wallType, positionIndex) => {
                    ...
                    if (aspectRatio > 1.5) {
                        paintingWidth = Math.min(
                            maxPaintingWidth,
                            basePaintingHeight * aspectRatio * 0.8
                        );
                    ...
                    // 创建画框
                    const paintingWithFrame = createPaintingFrame(
                        painting,
                        paintingWidth,
                        paintingHeight
                    );
                    ...
                    switch (wallType) {
                        case 'backWall':
                            ...
                            paintingWithFrame.position.set(
                                -10 + positionIndex * 10,
                                2.4,
                                -backWallOffset
                            );
                        ...
                        case 'rightWall': {
                            ...
                            const { x, y, z } = getRightWallHardcodedPosition(
                                imageData.item,
                                wallAssignments.rightWall
                            );
                        ...
                        case 'leftWall': {
                            ...
                            const { x, y, z } = getLeftWallHardcodedPosition(
                                imageData.item,
                                wallAssignments.leftWall
                            );
                            paintingWithFrame.position.set(x, y, z);
                    ...
                    // 优先使用预加载的纹理
                    console.log(
                        `🔍 查找纹理 - item.id: ${item.id}, 预加载纹理数量: ${galleryTextureManagerRef.current?.textures.size || 0}`
                    );
                    console.log(
                        `🔍 预加载纹理keys:`,
                        Array.from(galleryTextureManagerRef.current?.textures.keys() || [])
                    );

                    // 检查图片纹理缓存
                    if (galleryTextureManagerRef.current?.textures.has(item.id)) {
                        const preloadedTexture = galleryTextureManagerRef.current.textures.get(
                            item.id
                        );
                    ...
                    // 检查视频纹理缓存
                        ...
                        const preloadedVideoTexture = galleryTextureManagerRef.current.videos.get(
                            item.id
                        );
                        ...
                        try {
                            ...
                            if (item.type === 'video') {
                                ...
                                console.log(
                                    `⚠️ 预加载纹理不可用，尝试单独加载: ${item.id} -> ${mappedFileName}`
                                );
                ...
                // 顺序创建画作，避免同时创建过多
                const allPaintings = [
                    ...wallAssignments.backWall.map((data, i) => ({
                        data,
                        wallType: 'backWall',
                        index: i,
                    })),
                    ...wallAssignments.rightWall.map((data, i) => ({
                        data,
                        wallType: 'rightWall',
                        index: i,
                    })),
                    ...wallAssignments.leftWall.map((data, i) => ({
                        data,
                        wallType: 'leftWall',
                        index: i,
                    })),
                    ...wallAssignments.frontWall.map((data, i) => ({
                        data,
                        wallType: 'frontWall',
                        index: i,
                    })),
            ...
            const getRightWallHardcodedPosition = (item, rightWallImages) => {
                ...
                if (item.layer === 'upper') {
                    const upperIndex = rightWallImages
                        .filter(img => img.item.layer === 'upper')
                    ...
                    const lowerIndex = rightWallImages
                        .filter(img => img.item.layer === 'lower')
                        .findIndex(img => img.item.id === item.id);
                    return {
                        x: rightWallOffset,
            ...
            const createPaintingSpotlight = paintingMesh => {
                ...
                // 判断是否为竖式画作（前后墙的画作）
                const isVerticalPainting =
                    Math.abs(rotation.y) < 0.1 || Math.abs(rotation.y - Math.PI) < 0.1;
                ...
                } else if (Math.abs(rotation.y - Math.PI) < 0.1) {
                    // 前墙
                    lightPos.set(position.x, lightHeight, position.z - offset);
                } else if (Math.abs(rotation.y + Math.PI / 2) < 0.1) {
                    // 右墙
        ...
        const checkCollision = camera => {
            ...
            for (const pillar of pillarPositions) {
                const distance = Math.sqrt(
                    Math.pow(position.x - pillar.x, 2) + Math.pow(position.z - pillar.z, 2)
                );
            ...
            spotlightsRef.current.forEach(spotlight => {
                ...
                if (distance <= data.activationDistance) {
                    ...
                    const proximityFactor = Math.max(
                        0,
                        (data.activationDistance - distance) / data.activationDistance
                    );
        ...
        const updateCameraSpotlight = cameraPosition => {
            ...
            if (camera) {
                ...
                // 目标点始终设置在摄像机正前方10米处
                const fixedTargetPosition = camera.position
                    .clone()
        ...
        const startIntroAnimation = () => {
            ...
            camera.position.set(
                phases.phase1.startPos.x,
                phases.phase1.startPos.y,
                phases.phase1.startPos.z
            );
            camera.lookAt(
                phases.phase1.startLookAt.x,
                phases.phase1.startLookAt.y,
                phases.phase1.startLookAt.z
            );
            ...
            const executePhase = phaseConfig => {
                ...
                camera.position.x =
                    phaseConfig.startPos.x +
                    (phaseConfig.endPos.x - phaseConfig.startPos.x) * easeInOutCubic;
                camera.position.y =
                    phaseConfig.startPos.y +
                    (phaseConfig.endPos.y - phaseConfig.startPos.y) * easeInOutCubic;
                camera.position.z =
                    phaseConfig.startPos.z +
                    (phaseConfig.endPos.z - phaseConfig.startPos.z) * easeInOutCubic;

                const currentLookAt = {
                    x:
                        phaseConfig.startLookAt.x +
                        (phaseConfig.endLookAt.x - phaseConfig.startLookAt.x) * easeInOutCubic,
                    y:
                        phaseConfig.startLookAt.y +
                        (phaseConfig.endLookAt.y - phaseConfig.startLookAt.y) * easeInOutCubic,
                    z:
                        phaseConfig.startLookAt.z +
                        (phaseConfig.endLookAt.z - phaseConfig.startLookAt.z) * easeInOutCubic,
                };
                        ...
                        if (phaseComplete) {
                            ...
                            if (controlsRef.current) {
                                ...
                                controlsRef.current.connect =
                                    controlsRef.current.connect.__original ||
                                    controlsRef.current.connect;
                            }
                            return;
                        }
                    ...
                    try {
                        ...
                        const textureResult = await textureSystem.loadSceneTextures('gallery', {
                            ...
                            onProgress: (progress, loaded, total) => {
                                console.log(
                                    `📦 Gallery纹理加载进度: ${loaded}/${total} (${Math.round(progress * 100)}%)`
                                );
                            },
                        });
                        ...
                        console.log(
                            `✅ Gallery纹理预加载完成: ${textureResult.textures.size}张图片, ${textureResult.videos.size}个视频`
                        );
                        ...
                        controls.connect = () => {
                            if (isIntroAnimationComplete) {
                                try {
                                    ...
                                    console.warn(
                                        'PointerLockControls connect failed:',
                                        connectError
                                    );
                                }
                            }
                ...
                */
                ...
                const addArtisticElements = scene => {
                    ...
                    const createWallLightBox = () => {
                        ...
                        const lightBoxGeometry = new THREE.BoxGeometry(
                            entranceWidth,
                            entranceHeight,
                            lightBoxDepth
                        );
                        ...
                        const createLightboxDisplay = () => {
                            ...
                            const lightboxItem = galleryData.find(
                                item => item.position === 'lightbox'
                            );
                            const adImagePath = lightboxItem
                                ? lightboxItem.src || lightboxItem.thumbnail
                                : lightboxConfig.imagePath;
                            ...
                            function loadImageFallback() {
                                ...
                                const lightboxBaseName = adImagePath
                                    .split('/')
                                    .pop()
                                    .replace(/\.(jpg|jpeg|png|webp|avif)$/i, '');

                                textureSystem
                                    .loadTexture(lightboxBaseName)
                                    .then(texture => {
                                        // 🖼️ 图片模式：正常材质 + 图片纹理
                ...
                const initializeRectAreaLighting = () => {
                    try {
                        ...
                        rectAreaLightingRef.current = new RectAreaLightingSystem(
                            scene,
                            rectLightConfig
                        );
                ...
                const initializeLightCubes = async () => {
                    try {
                        ...
                        pillarLightRef.current = new PillarLightSystem(
                            scene,
                            cubeConfig,
                            galleryData
                        );
                ...
                const initializeIESSpotlights = () => {
                    try {
                        ...
                        // 创建IES聚光灯系统实例
                        iesSpotlightSystemRef.current = new IESSpotlightSystem(
                            scene,
                            renderer,
                            iesConfig
                        );
                ...
                const animate = () => {
                    ...
                    scene.traverse(object => {
                        if (
                            object.material &&
                            object.material.map &&
                            object.material.map.isVideoTexture
                        ) {
        ...
        <>
                    ...
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 text-center max-w-md border border-white/20">
                        <div className="mb-6">
                            ...
                            <h3 className="text-2xl font-bold text-white mb-2">
                                {texts[language]?.gallery?.mobile?.mobileNotice?.title ||
                                    (language === 'zh'
                                        ? '移动端体验提示'
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                {texts[language]?.gallery?.mobile?.mobileNotice?.viewGallery ||
            ...
            {!isMobile && (
                ...
                >
                    ...
                    <div
                        className={`absolute bottom-6 left-6 bg-black/50 backdrop-blur-md rounded-xl p-4 text-white z-20 max-w-sm transition-all duration-1000 ${
                            !isLoading &&
                    ...
                    >
                        <p className="text-lg font-medium mb-3">
                            {texts[language]?.gallery?.gallery3D?.instructions?.howToPlay ||
                                (language === 'zh' ? '如何操作？' : 'How to Play?')}
                        </p>
                        <div className="space-y-3 text-sm">
                            <p className="flex items-center">
                                <span className="w-2"></span>
                                {texts[language]?.gallery?.gallery3D?.instructions?.clickToEnter ||
                                    (language === 'zh'
                                        ? '点击进入长廊'
                            ...
                            <p className="flex items-center">
                                <span className="w-2"></span>
                                {texts[language]?.gallery?.gallery3D?.instructions?.mouseControl ||
                                    (language === 'zh'
                                        ? '鼠标 - 环视周围，探索画作'
                            ...
                            <p className="flex items-center">
                                ...
                                <span className="inline-flex items-center gap-1 mr-2">
                                    <span className="inline-flex items-center px-1.5 py-0.5 bg-white/20 rounded text-xs font-mono border border-white/30">
                                        W
                                    </span>
                                    <span className="inline-flex items-center px-1.5 py-0.5 bg-white/20 rounded text-xs font-mono border border-white/30">
                                        A
                                ...
                                <span>
                                    {texts[language]?.gallery?.gallery3D?.instructions
                                        ?.keyboardControl ||
                                        (language === 'zh'
                                            ? '移动穿行长廊'
                            ...
                            <p className="flex items-center">
                                <span className="w-2"></span>
                                <span className="inline-flex items-center px-2 py-0.5 mr-2 bg-white/20 rounded text-xs font-mono border border-white/30">
                                    ESC
                                </span>
                                <span>
                                    {texts[language]?.gallery?.gallery3D?.instructions
                                ...
                                if (controlsRef.current?.lock) {
                                    try {
                                        ...
                                        setTimeout(() => {
                                            if (!isPointerLocked) {
                                                console.log(
                                                    'Fallback: manually setting pointer locked state'
                                                );
                                                setIsPointerLocked(true);
                                            }
                                        }, 100);
                                        ...
                                        console.log(
                                            'Pointer lock failed, using fallback:',
                                            err.message
                                        );
                                        setIsPointerLocked(true);
                                    }
