...
export class IESSpotlightSystem {
    ...
    async createIESSpotlights(IESSpotLight) {
        ...
        for (let i = 0; i < this.config.lightCount; i++) {
            ...
            console.log(`💡 创建IES聚光灯 ${i + 1}: 位置(${position.x}, ${position.y}, ${position.z}) -> 目标(${target.x}, ${target.y}, ${target.z})`);
        }
    }
    ...
    createStandardSpotlights() {
        for (let i = 0; i < this.config.lightCount; i++) {
            ...
            // 创建标准聚光灯
            const spotlight = new THREE.SpotLight(color, this.config.intensity, this.config.distance, this.config.angle, this.config.penumbra);
            spotlight.position.set(position.x, position.y, position.z);
            spotlight.castShadow = true;
            ...
            console.log(`💡 创建标准聚光灯 ${i + 1}: 位置(${position.x}, ${position.y}, ${position.z}) -> 目标(${target.x}, ${target.y}, ${target.z})`);
        }
    }
    ...
    updateAnimation() {
        ...
        this.spotlights.forEach((spotlightData, index) => {
            ...
            light.intensity = spotlightData.originalIntensity * (intensityBase + intensityVariation);
            ...
            // 亮度变化
            const newLightness = Math.max(0.4, Math.min(0.8, hsl.l + Math.sin(timeOffset * 2) * 0.2));
