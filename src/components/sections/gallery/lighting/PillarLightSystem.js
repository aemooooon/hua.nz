...
export class PillarLightSystem {
    ...
    createLightCylinders() {
        ...
        console.log(
            `✅ 所有 ${this.config.positions.length} 个发光圆柱体创建完成（纹理将延迟加载）`
        );
    }
    ...
    createSingleLightCylinder(posConfig, index) {
        ...
        if (posConfig.rotationY !== undefined) {
            ...
            console.log(
                `🔄 柱子绕Y轴旋转: ${((posConfig.rotationY * 180) / Math.PI).toFixed(1)}度`
            );
        }
        ...
        console.log(
            `🏛️ 创建发光圆柱体 ${index + 1}: ${positionDesc[index]} 位置(${posConfig.x}, ${centerY}, ${posConfig.z}), 高度${this.config.height}米, 半径${cylinderRadius}米, 直径${(cylinderRadius * 2).toFixed(3)}米`
        );
        ...
        if (cylinderIndex >= 0 && cylinderIndex < this.lightCylinders.length) {
            ...
            if (textureUrl) {
                ...
                const newMaterial = new THREE.MeshStandardMaterial({
                    ...
                    transparent: true,
                    opacity: 0.9,
                    emissive: new THREE.Color(cylinderData.config.color).multiplyScalar(
                        this.config.emissiveIntensity
                    ),
                    side: THREE.DoubleSide,
                });
    ...
    updateAnimation() {
        ...
        this.lightCylinders.forEach(cylinderData => {
            if (cylinderData.light) {
                ...
                const pulse =
                    Math.sin(
                        time * this.config.animationSpeed * 1000 + cylinderData.animationPhase
                    ) *
                        0.3 +
                    0.7;
                ...
                const colorShift =
                    Math.sin(
                        time * this.config.animationSpeed * 500 + cylinderData.animationPhase
                    ) * 0.1;
                const hsl = {};
                ...
                if (cylinderData.mesh && cylinderData.mesh.material) {
                    ...
                    if (!cylinderData.mesh.material.map) {
                        ...
                        if (cylinderData.mesh.material.emissive) {
                            cylinderData.mesh.material.emissive.copy(
                                newColor.clone().multiplyScalar(this.config.emissiveIntensity)
                            );
                        }
                    }
                }
    ...
    async loadGalleryTexture(baseName) {
        ...
        try {
            ...
            console.log(
                `🎯 当前浏览器支持格式: ${compressionInfo.format} (${compressionInfo.description})`
            );
