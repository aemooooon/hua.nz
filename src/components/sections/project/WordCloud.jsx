...
const WordCloud = ({ 
  ...
  // 计算最适合的高度 - 简化版本
  const calculateOptimalHeight = useCallback((words, containerWidth) => {
  ...
  const generateWordCloud = useCallback(() => {
    ...
    try {
      ...
      // 基础信息
      const basicInfo = [];
      if (project.year) basicInfo.push({ 
        word: project.year.toString(), 
        category: 'Year', 
        importance: 2 
      });
      if (project.location) basicInfo.push({ 
        word: project.location, 
        category: 'Location', 
        importance: 2 
      });
      if (project.company) basicInfo.push({ 
    ...
    layout.start();
      ...
      function draw(words) {
        const g = svg.append("g")
          .attr("transform", `translate(${actualWidth / 2},${optimalHeight / 2})`);      const texts = g.selectAll("text")
        .data(words)
        .enter().append("text")
        .style("font-size", d => `${d.size}px`)
        .style("font-family", "Inter, system-ui, sans-serif")
        .style("font-weight", d => d.importance >= 4 ? "600" : d.importance >= 3 ? "500" : "400")
