...
const ProjectDetailNew = ({ project = null, isOpen, onClose }) => {
  ...
  const getCategoryColors = (type) => {
    const colorMap = {
      'Data Science': { text: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', hover: 'hover:bg-blue-500/20' },
      'Backend': { text: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/30', hover: 'hover:bg-red-500/20' },
  ...
  // 添加 shader 效果初始化
  useEffect(() => {
    // 加载 Three.js 并初始化 shader 效果
    const initShader = () => {
      window.initShaderEffect = (imageElement) => {
        ...
        // Fragment shader with pixel grid and distortion
        const fragmentShader = `
          ...
          void main() {
            ...
            vec2 centerOfPixel = gridUV + vec2(1.0/25.0, 1.0/25.0);
            
            vec2 mouseDirection = u_mouse - u_prevMouse;
            vec2 pixelToMouseDirection = centerOfPixel - u_mouse;
            float pixelDistanceToMouse = length(pixelToMouseDirection);
            float strength = smoothstep(0.25, 0.0, pixelDistanceToMouse);
        ...
        try {
          ...
          console.warn('Shader effect initialization failed:', error);
        }
      };
    };
    ...
    return () => {
      if (window.initShaderEffect) {
        delete window.initShaderEffect;
      }
    };
  }, []);
    ...
    return (
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          <svg className="w-5 h-5 text-theme-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="mb-12 lg:mb-16">
            ...
            {hasMultipleImages && (
              <div className="space-y-4">
                ...
                {/* Main Display Image - Ultra wide aspect ratio with Shader Effect */}
                <div className="relative group">
                  <div className="aspect-[16/7] lg:aspect-[21/8] overflow-hidden rounded-2xl shadow-2xl shader-image-container">
                    <img
                      ...
                      onLoad={(e) => {
                        // Initialize shader effect when image loads
                        setTimeout(() => {
                          if (window.initShaderEffect) {
                            console.log('Initializing shader effect for image:', e.target.src);
                            window.initShaderEffect(e.target);
                          } else {
                            console.warn('Shader effect function not available');
                          }
                        }, 100);
                      }}
                    />
              ...
              <div className="space-y-3">
                ...
                {project.links?.live && (
                  ...
                  >
                    ...
                    <span className="font-semibold">{projectText.liveDemo}</span>
                  </a>
                )}
                ...
                {project.links?.github && (
                  ...
                  >
                    <ExternalLink className="w-5 h-5 group-hover:scale-110 transition-transform flex-shrink-0" />
                    <span className="font-semibold">{projectText.githubRepo}</span>
                  </a>
                )}
              </div>
            )}
