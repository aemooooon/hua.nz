...
const ProjectGeoViewer = ({ isOpen, onClose, language = 'en' }) => {
    ...
    // 从store获取项目数据
    const { getAllFootprints, getProjectsText } = useAppStore();
    ...
    const projects = useMemo(() => {
        return getAllFootprints().filter(
    ...
    const createCustomControls = useCallback(() => {
        ...
        projects,
        calculateCentroid,
        projectText.map.resetToDefaultView,
        ...
        if (isOpen && mapRef.current && !mapInstanceRef.current) {
            ...
            projects.forEach(project => {
                ...
                marker.on('mouseover', function () {
                    ...
                    marker
                        .bindTooltip(
                            getBilingualText(project.title) || getBilingualText(project.name),
                            {
                                permanent: false,
                                direction: 'top',
                                offset: [0, -25],
                    ...
                    if (markerElement) {
                        ...
                        if (innerDiv) {
                            innerDiv.style.filter =
                                'brightness(1.2) contrast(1.1) drop-shadow(0 6px 16px rgba(0,0,0,0.5))';
                            innerDiv.style.borderWidth = '4px';
                            innerDiv.style.boxShadow =
                                '0 6px 20px rgba(0,0,0,0.6), inset 0 3px 6px rgba(255,255,255,0.3)';
                        }
                    }
                });
      ...
      `;
        ...
        return () => {
            if (!isOpen && mapInstanceRef.current) {
                ...
                // 清理自定义样式
                if (
                    mapInstanceRef.current._customStyle &&
                    mapInstanceRef.current._customStyle.parentNode
                ) {
                    mapInstanceRef.current._customStyle.parentNode.removeChild(
                        mapInstanceRef.current._customStyle
                    );
                }
