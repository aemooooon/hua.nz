...
export const usePerformanceMonitor = (options = {}) => {
    ...
    // 更新性能数据
    const updatePerformance = useCallback((currentTime = performance.now()) => {
        if (!enabled) return;
        ...
        if (deltaTime > 0) {
            ...
            if (perf.frameCount % 10 === 0) {
                ...
                // 性能变化回调
                if (onPerformanceChange && (perf.isWarning !== isWarning || perf.isCritical !== isCritical)) {
    ...
    }, [enabled, sampleSize, warningFPSThreshold, criticalFPSThreshold, onPerformanceChange, getMemoryUsage, generatePerformanceSuggestions]);
    ...
    // 性能建议生成
    const generatePerformanceSuggestions = useCallback(({ averageFPS, memoryUsage, isWarning, isCritical }) => {
        const suggestions = [];
    ...
    const getDetailedReport = useCallback(() => {
        ...
        return {
            ...
            timing: {
                frameTime: perf.frameTimes.length > 0 ? 
                    perf.frameTimes.reduce((sum, time) => sum + time, 0) / perf.frameTimes.length : 0,
                renderTime: perf.renderTime,
            ...
            system: {
                frameCount: perf.frameCount,
                uptime: performance.now() - (perf.lastTime - (perf.frameTimes[perf.frameTimes.length - 1] || 0)),
